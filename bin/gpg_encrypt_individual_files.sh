...
#a space separated list of recipient key IDs; this value can be overridden by environment
recipient_list="${recipient_list:-}"

#remove the original unencrypted file?; this value can be overridden by environment
remove_original="${remove_original:-true}"

#skip files that already have an encrypted equivalent? i.e. this will replace the encrypted file equivalent if it already exists; this value can be overridden by environment
skip_encrypted="${skip_encrypted:-false}"

#create checksums for the files in each directory.  Each directory will contain a file call sha1sum.txt; this value can be overridden by environment
...
#this will individually encrypt all files in the folder; this value can be overridden by environment
if [ -z "${folder_to_encrypt}" ];then
  folder_to_encrypt="${1}"
fi

#if the end of the file or name matches an ignore_rule then it will not be encrypted; this value can be overridden by environment
if [ "${#ignore_rules[@]}" -eq "0" ];then
  ignore_rules=('.gpg' 'sha1sum.txt' '.checksumrequired')
fi
...
ignore_expression=""
for x in ${ignore_rules[@]};do ignore_expression="${ignore_expression} -path *${x} -prune -o "; done

#use find command to find files to encrypt
find "${folder_to_encrypt}" ${ignore_expression} -type f -print | while read x;do
  dir="${x%/*}"
  file="${x##*/}"
  (
    #subshell
    ...
    if ! ${skip_encrypted} || [ "${skip_encrypted}" = "true" -a ! -f "${file}.gpg" ];then
      #sign (-d) encrypt (-e) the file (output is filename.gpg)
      if ${sign_encrypted_file};then
        gpg -s -e ${recipients} -- "${file}"
      else
        gpg -e ${recipients} -- "${file}"
      fi
      echo -n "encrypted ${x}"
...
#create a checksum of each file in the folder but ONLY do it to encrypted files that have changed (i.e. has a *.gpg.checksumrequired file)
${create_checksums} && find "${folder_to_encrypt}" -type d | while read x;do
  (
    ...
    find . -maxdepth 1 -type f -name '*.gpg.checksumrequired' -printf '%f\n' | while read esum;do
      ...
      expression="$(echo "${esum%\.checksumrequired}" | sed 's/\[/\\\[/g' | sed 's/\]/\\\]/g')"
